...
class Scheduler(object):
    ...
    def start(self, base_dir, window, fn_start, fn_stop, fn_counter, fn_error):
        self.window = window
        self.terminate = False
        # 1、 获取当前表格中的所有数据 ，每一行创建一个线程去执行监控
        for row_index in range(window.table_widget.rowCount()):
            ...
            t.error_counter_signal.connect(fn_error)
            t.start()
            # 将创建的线程添加到线程列表中
    ...
    def stop(self):
        ...
        # 创建一个线程，去监测thread_list中的数量，实时更新到窗体中的Label中
        # self.window.update_status_message("xxx")
        from utils.threads import StopThread
        t = StopThread(self, self.window)
        t.updated_signal.connect(self.window.update_status_message)
        t.start()

    def destory_thread(self, thread):
